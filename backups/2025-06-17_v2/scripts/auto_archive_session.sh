...
main() {
    ...
    case "$mode" in
        ...
        "force")
            ...
            ;;
        "auto")
            echo -e "${YELLOW}⚠️ Automatic archival has been disabled.${NC}"
            echo -e "${BLUE}💾 Use manual backup system instead:${NC}"
            echo -e "    scripts/manual_backup.sh backup           - Standard backup"
            echo -e "    scripts/manual_backup.sh full backup      - Comprehensive backup"
            show_archive_status
            ;;
        "status")
            show_archive_status
