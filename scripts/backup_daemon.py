...
class BackupDaemon:
    ...
    def _check_and_enforce_backup(self):
        """Check if backup is due and enforce 120-minute rule."""
        try:
            ...
            if self.backup_system.check_backup_due():
                ...
                minutes_since = status.get('minutes_since_backup', 0)
                
                # Only log status every 12 checks (about once per hour)
                ...
                if self._status_check_count % 12 == 0:
                    logger.info(f"Backup status: {minutes_since:.1f} minutes since last backup (120-minute enforcement)")
